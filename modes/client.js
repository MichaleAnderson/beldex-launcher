...
function clientConnect(config) {
  ...
  const client = net.createConnection({ path: socketPath }, () => {
    ...
    console.log('Remember to use ctrl-c to exit the client without shutting down the Master Node.')
  })
  ...
  stdin.on('data', function(str) {
    ...
    if (lastcommand.trim() == "exit") {
      console.log("SHUTTING DOWN MASTER NODE and this client, will end when Master Node has shutdown.")
      // FIXME: prompt
    }
