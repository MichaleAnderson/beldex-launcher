    ...
    lib.waitForLauncherStop(config, function() {
      ...
      uidGetter.uidNumber(user, function(err, uid, homedir) {
        ...
        if (config.blockchain.data_dir_is_default) {
          ...
          config.blockchain.data_dir = configUtil.getBeldexDataDir(config)
        }
        ...
        if (fs.existsSync(config.blockchain.binary_path)) {
          ...
          console.warn('Warning your beldexd does not exist at', config.blockchain.binary_path, ', recommend running download-binaries or obtain them off github')
        }
        if (config.network.binary_path) {
          if (fs.existsSync(config.network.binary_path)) {
            fs.chownSync(config.network.binary_path, uid, 0)
          } else {
            console.warn('Warning your belnet does not exist at', config.network.binary_path, ', recommend running download-binaries or obtain them off github')
          }
        }
        if (config.storage.binary_path) {
          if (fs.existsSync(config.storage.binary_path)) {
            ...
            console.warn('Warning your beldex-storage does not exist at', config.storage.binary_path, ', recommend running download-binaries or obtain them off github')
          }
        }
        ...
        if (fs.existsSync(config.launcher.var_path)) {
          ...
          fs.chownSync(config.launcher.var_path, uid, 0)
          // should also make /opt/beldex-launcher not root
          // but not /var
          if (config.launcher.var_path == '/opt/beldex-launcher/var') {
            fs.chownSync('/opt/beldex-launcher', uid, 0)
          }
        }
        ...
        if (config.network.data_dir) {
          ...
          if (!fs.existsSync(config.network.data_dir)) {
            belnet.mkDirByPathSync(config.network.data_dir)
          }
          fs.chownSync(config.network.data_dir, uid, 0)
        }
        // config.network.belnet_nodedb
        if (config.network.belnet_nodedb) fs.chownSync(config.network.belnet_nodedb, uid, 0)
        if (config.network.enabled) {
          if (fs.existsSync(config.launcher.var_path + '/belnet.version')) {
            fs.chownSync(config.launcher.var_path + '/belnet.version', uid, 0)
          }
          if (fs.existsSync(config.launcher.var_path + '/mnode_address')) {
            fs.chownSync(config.launcher.var_path + '/mnode_address', uid, 0)
          }
              ...
              exec('getcap ' + config.network.binary_path, function (error, stdout, stderr) {
                ...
                if (!(stdout.match(/cap_net_bind_service/) && stdout.match(/cap_net_admin/))) {
                  if (process.getgid() != 0) {
                    ...
                    // are root
                    console.log('going to try to setcap your belnet binary, so you don\'t need to run as root')
                    try {
        ...
        fs.chownSync('/opt/beldex-launcher/bin', uid, 0)
