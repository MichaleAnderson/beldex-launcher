...
const belnet = require(__dirname + '/../belnet') // expects 0.8 used for randomString
  ...
  return new Promise(resolve => {
    ...
    function getFreeSpaceUnix(path, cb) {
      ...
      child_process.exec('df -kP ' + path, function(error, stdout) {
        ...
        if (!calledBack) {
          console.error('Could not parse your df output, please create a github issue here: https://github.com/beldex-project/beldex-launcher/issues with the following: ', stdout)
    ...
    var mnode_problems = 0
    var mnode_warnings = 0
    var need = {
      diskspace: false,
      rpcport: false,
    }
    ...
    function markCheckDone(doneSubSystem) {
      ...
      if (mnode_problems) {
        console.error('We have found', mnode_problems, 'issue(s), please address them before running a master node')
        console.log('')
    ...
    function diskspaceCheck() {
      ...
      if (os.platform() == 'linux') {
        ...
        if (!fs.existsSync('/dev/net')) {
          log.push('WE COULD NOT VERIFY THAT YOU HAVE /dev/net, this is now required to run a master node. Ask your service provider to enable it or you may need to find a new provider.')
    ...
    function portTest() {
      ...
      networkTest.createClient('na.testing.belnet.org', 3000, async function(client) {
        if (client === false) {
