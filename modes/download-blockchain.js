    ...
    */
    ...
    function checkDone(label, value) {
      ...
      if (Object.keys(pingMap).length === 2) {
        // https://imaginary.stream/beldex/data.mdb
        // https://public.beldex.foundation/beldex/data.mdb
        // https://public.beldex.foundation/beldex/data.mdb.md5sum
        let url = 'https://' + (pingMap['ca'] > pingMap['eu'] ? 'public.beldex.foundation' : 'imaginary.stream')
        url += '/beldex/data.mdb'
    ...
    const eu = cp.execFile('/bin/ping', ['-n', '-c5', 'public.beldex.foundation'], function(error, stdout, stderr) {
      if (error) console.error(error)
      var avg = []
      for(line of stdout.split('\n')) {
        if (line.match(/time=/)) {
