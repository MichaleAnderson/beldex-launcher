...
async function continueStart() {
  ...
  // centos8
  stripArg('/bin/node')
  stripArg('/bin/beldex-launcher')
  stripArg(__filename) // will just be index.js
  stripArg('beldex-launcher')
  stripArg('/usr/bin/beldex-launcher')
  stripArg('/usr/local/bin/beldex-launcher')
  // how is this not __filename??
  stripArg('/usr/lib/node_modules/beldex-launcher/index.js')
  ...
  if (fs.existsSync('/etc/beldex-launcher/launcher.ini')) {
    const ini_bytes = fs.readFileSync('/etc/beldex-launcher/launcher.ini')
  ...
  if (showVersionCommands.includes(mode)) {
    if (useGitVersion) {
      console.log('Beldex-Launcher version', VERSION.toString())
    } else {
      console.log('Beldex-Launcher version', VERSION.toString())
    }
  }
  ...
  async function doInstallUpgrade(user, config, install) {
    ...
    if (systemdUtils.isSystemdEnabled(config)) {
      const stdout = execSync('systemctl start beldexd')
    } else {
      // override args
      // not yet...
    ...
    // console.log('Or alternative please run: sudo systemctl start beldexd')
  ...
  async function getPubKeys() {
    ...
    if (running.beldexd) {
      ...
      const belnet = require('./belnet')
      keys = await lib.runOfflineBlockchainRPC( daemon, belnet, config, lib.blockchainRpcGetKey)
    }
    delete keys.status
    return keys
  }
  ...
  switch(mode) {
    ...
    case 'status': // official
      ...
      if (type) {
        ...
        switch(type) {
          case 'blockchain':
            // can hang if beldexd is popping blocks
            ...
            if (pubkey) {
              console.log(`More info: https://beldexsn.com/sn/${pubkey}`)
            }
          break;
          case 'storage':
    ...
    case 'show-quorum':
      ...
      if (!running.beldexd) {
        ...
        await lib.startBeldexdForRPC(daemon, belnet, config)
      }

      keys = await lib.blockchainRpcGetKey(config)
      if (!keys || !keys.result || !keys.result.master_node_pubkey) {
        console.error('Could not get our pubkey')
        return
      }
      const ourPubKey = keys.result.master_node_pubkey
    ...
    case 'stop': // official
      // maybe use the client to see what's taking beldexd a while...
      ...
      if (!pid) {
        ...
        const belnet = require('./belnet')
        // TODO: belnet running? can't check udp. beldex-storage port running?
        // TODO: launcher actually running somewhere?
        // and is it locked up? webserver port?
        belnet.portIsFree(config.blockchain.rpc_ip, config.blockchain.rpc_port, async function(portFree) {
          if (!portFree) {
            const portPid = await lib.findPidByPort(config.blockchain.rpc_port)
            console.log('')
            console.log(`There's a beldexd on ${portPid} that we're not tracking using our configuration (rpc_port is already in use). You likely will want to confirm and manually stop it before start using the launcher again.`);
            console.log('')
          }
        })
      }
    ...
    case 'config-view': // official
      console.log('Beldex-launcher is in', __dirname)
      // FIXME: prettyPrint
    ...
    case 'systemd':
      ...
      if (type === 'disable') {
        requireRoot()
        // unlink('/etc/systemd/system/beldexd.service')
      } else
      if (type === 'log') {
    ...
    case 'download-binaries': // official
      // because of belnet and mkdirp /opt/...
      requireRoot()
    ...
    case 'export': // official
      ...
      if (!fs.existsSync(config.blockchain.beldexd_key)) {
        console.log(config.blockchain.beldexd_key, "does not exist, this has not been run as a master node yet")
        process.exit()
      }
      ...
      if (!opt1) {
        ...
        key = keys.pubkey || keys.master_node_pubkey
      }
  ...
  beldex-launcher is manages the Beldex.network suite of software primarily for master node operation
  Usage:
    [sudo] beldex-launcher [command] [OPTIONS]

    Commands:
      start       start the beldex suite with OPTIONS
      stop        stops the launcher running in the background
      status      get the current beldex suite status, can optionally provide:
                    blockchain - get blockchain status
      client      connect to beldexd
      prequal     prequalify your server for master node operation
      config-view print out current configuration information
      versions    show installed versions of Beldex software
      systemd     requires one of the following options
                    log - show systemd launcher log file
      keys        get your master node public keys
      show-quorum tries to give an estimate to the next time you're tested
      export [FILENAME]   try to create a compressed tarball with all the mnode files
                            that you need to migrate this mnode to another host
      ...
      download-blockchain deletes current blockchain and downloads a fresh sync'd copy
                            usually much faster than a normal beldexd sync takes

    Commands that require root/sudo:
      download-binaries - download the latest version of the beldex software suite
        can optionally provide: force, prerel and force-prerel
      upgrade-systemd (check-systemd) - reconfigures your beldexd.service
