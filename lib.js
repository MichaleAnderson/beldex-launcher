...
function getBlockchainVersion(config) {
  ...
  if (config.blockchain.binary_path && fs.existsSync(config.blockchain.binary_path)) {
    try {
      ...
      //console.log('beldexd_version', beldexd_version)
      // Beldex 'Nimble Nerthus' (v6.1.4-6f78319d0)
      return blockchainVersion
    } catch(e) {
      /*
Cant detect blockchain version Error: Command failed: /opt/beldex-launcher/bin/beldexd --version
/opt/beldex-launcher/bin/beldexd: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.25' not found (required by /opt/beldex-launcher/bin/beldexd)

    at checkExecSyncError (child_process.js:621:11)
    at execFileSync (child_process.js:639:15)
    at Object.getBlockchainVersion (/root/beldex-daemon-launcher/lib.js:66:22)
    at getBeldexdVersion (/root/beldex-daemon-launcher/config.js:291:31)
    at isBlockchainBinary3X (/root/beldex-daemon-launcher/config.js:312:3)
    at checkLauncherConfig (/root/beldex-daemon-launcher/config.js:347:9)
    at Object.checkConfig [as check] (/root/beldex-daemon-launcher/config.js:821:3)
    at continueStart (/root/beldex-daemon-launcher/index.js:125:14)
    at Object.<anonymous> (/root/beldex-daemon-launcher/index.js:22:3)
...
function getStorageVersion(config) {
  ...
  if (config.storage.binary_path && fs.existsSync(config.storage.binary_path)) {
    try {
      ...
      //const tempdir = belnet.randomString(8)
      let tempDir = '/tmp/beldex-storage-vercheck-' + config.storage.port
      ...
      if (fs.existsSync(tempPath)) {
        ...
        try {
          ...
          tempDir = '/tmp/beldex-storage-vercheck-' + belnet.randomString(8)
      ...
      // [2020-03-12 07:53:16.940] [info] [print_version] Beldex Storage Server v1.0.10
      if (lines[3].match(/Beldex Storage Server v/)) {
  ...
  if (fs.existsSync(config.launcher.var_path + '/launcher.pid')) {
    ...
    if (pid && isPidRunning(pid)) {
      ...
      for(var i in lines) {
        ...
        if (thisPid == pid) {
          foundPid = true
          // /usr/local/bin/node   /Users/admin/Sites/beldex-daemon-launcher/index.js ...
          //console.log(thisPid, 'cmd', cmd)
          if (cmd.match(/node/) || cmd.match(/index\.js/) || cmd.match(/beldex-launcher/)) {
            verifiedPid = true
          }
        }
      }
      ...
      if (!verifiedPid) {
        ...
        const blockchainIsRunning = pids.beldexd && isPidRunning(pids.beldexd)
        const networkIsRunning = config.network.enabled && pids.belnet && isPidRunning(pids.belnet)
...
function savePids(config, args, beldex_daemon, belnet, storageServer) {
  var obj = {
    runningConfig: config,
    arg: args,
    launcher: process.pid
  }
  if (beldex_daemon && !beldex_daemon.killed && beldex_daemon.pid) {
    obj.beldexd = beldex_daemon.pid
    obj.blockchain_startTime      = beldex_daemon.startTime
    obj.blockchain_startedOptions = beldex_daemon.startedOptions
    obj.blockchain_spawn_file     = beldex_daemon.spawnfile
    obj.blockchain_spawn_args     = beldex_daemon.spawnargs
  }
  ...
  var belnetPID = belnet.getPID()
  if (belnetPID) {
    var belnet_daemon = belnet.getBelnetDaemonObj()
    obj.belnet            = belnetPID
    obj.network_startTime  = belnet_daemon.startTime
    obj.network_spawn_file = belnet_daemon.spawnfile
    obj.network_spawn_args = belnet_daemon.spawnargs
    obj.network_blockchain_failures = belnet_daemon.blockchainFailures
  }
...
function getProcessState(config) {
  ...
  if (pids.beldexd && isPidRunning(pids.beldexd)) {
    //console.log("LAUNCHER: old beldexd is still running", pids.beldexd)
    running.beldexd = pids.beldexd
  }
  // console.log('network', config.network.enabled, 'belnet', pids.belnet)
  if (config.network.enabled) {
    if (pids.belnet && isPidRunning(pids.belnet)) {
      //console.log("LAUNCHER: old belnet is still running", pids.belnet)
      running.belnet = pids.belnet
    }
  }
...
async function runStorageRPCTest(belnet, config, cb) {
  ...
  const data =  await belnet.httpGet(url)
...
async function getLauncherStatus(config, belnet, offlineMessage, cb) {
  var checklist = {}
  ...
  checklist.blockchain = running.beldexd ? ('running as ' + running.beldexd) : offlineMessage
  ...
  if (pids.runningConfig.network.enabled || pids.runningConfig.storage.enabled) {
    if (running.beldexd) {
      //console.log('beldexd_key', config.storage.beldexd_key)
      checklist.beldexKey = fs.existsSync(pids.runningConfig.blockchain.beldexd_key) ? ('found at ' + pids.runningConfig.blockchain.beldexd_key) : offlineMessage
      //checklist.beldexEdKey = fs.existsSync(pids.runningConfig.blockchain.beldexd_edkey) ? ('found at ' + pids.runningConfig.blockchain.beldexd_edkey) : offlineMessage
    }
  }

  if (pids.runningConfig.network.enabled) {
    checklist.network = running.belnet ? ('running as ' + running.belnet) : offlineMessage
    // belnet rpc check?
  }
  ...
  if (running.beldexd) {
    ...
    //console.log('Beldexd is running, checking to make sure it\'s responding')
    ...
    var p = belnet.httpGet(url, function(data) {
      if (responded) return
      responded = true
  ...
  if (pids.runningConfig && pids.runningConfig.blockchain) {
    need.blockchain_rpc = false
    belnet.portIsFree(pids.runningConfig.blockchain.rpc_ip, pids.runningConfig.blockchain.rpc_port, function(portFree) {
  ...
  */
  ...
  if (pids.runningConfig.storage.enabled && running.storageServer) {
    ...
    function runStorageTest() {
      runStorageRPCTest(belnet, pids.runningConfig, function(data) {
        if (data === undefined) {
    ...
    if (pids.runningConfig.storage.ip === undefined) {
      ...
      // don't belnet running to use the belnet interface
      ...
      belnet.checkConfig() // set up test config for getNetworkIP
      belnet.getNetworkIP(function(err, localIP) {
        if (err) console.error('lib::getLauncherStatus - belnet.getNetworkIP', err)
  ...
  //checklist.push('socket', pids.beldexd?'running':offlineMessage)
  ...
  if (pids.runningConfig.network.enabled) {
    ...
    if (pids.network_blockchain_failures && pids.network_blockchain_failures.last_blockchain_mnode) {
      checklist.network_last_failure_blockchain_mnode = new Date(pids.network_blockchain_failures.last_blockchain_mnode)+''
    }

    // if belnet rpc is enabled...
...
// only stop beldexd, which should stop any launcher
function stopBeldexd(config) {
  var running = getProcessState(config)
  if (running.beldexd) {
    var pids = getPids(config)
    console.log('blockchain is running, requesting shutdown')
    // can't use 15
    process.kill(pids.beldexd, 'SIGTERM')
    return 1
  }
  return 0
}
...
function stopLauncher(config) {
  ...
  if (systemdUtils.isSystemdEnabled(config)) {
    console.log('systemd beldexd service is enabled')
    if (systemdUtils.isStartedWithSystemD()) {
      console.log('systemd beldexd service is active, stopping')
      // are we root?
      if (process.getuid() !== 0) {
        console.log("this command isn't running as root, so can't stop launcher, run again with sudo")
      } else {
        try {
          const stdoutBuf = execSync('systemctl stop beldexd')
  ...
  if (pid) {
    ...
    if (systemdUtils.isStartedWithSystemD()) {
      ...
      console.warn('"sudo systemctl stop beldexd.service" before running this')
    ...
    if (config.network.enabled && running.belnet) {
      ...
      process.kill(pids.belnet, 'SIGTERM') // 15
      count++
    }
  }
  return count
...
function waitForLauncherStop(config, cb) {
  var running = getProcessState(config)
  if (running.beldexd || running.belnet || running.storageServer) {
    var wait = 500
    if (running.beldexd) wait += 4500
    setTimeout(function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    const req = protoClient.request({
      ...
      headers: {
        ...
        'Host': 'localhost', // hack for belnet
...
async function startBeldexdForRPC(daemon, belnet, config) {
  daemon.config = config // update config for shutdownEverything

  belnet.disableLogging(true)
  const publicIPv4 = await belnet.getPublicIPv4()
  if (!publicIPv4) {
  ...
  const args = []
  const parameters = daemon.configureBeldexd(config, args)
  const beldexd_options = parameters.beldexd_options
  //console.log('configured ', config.blockchain.binary_path, beldexd_options.join(' '))
  config.blockchain.quiet = true // force quiet
  daemon.launchBeldexd(config.blockchain.binary_path, beldexd_options, false, config, args)
  await waitForBlockchain(config, { timeout: 30 * 1000 })
}

async function runOfflineBlockchainRPC(daemon, belnet, config, rpcFunc) {
  startBeldexdForRPC(daemon, belnet, config)
  await waitForBlockchain(config, { timeout: 30 * 1000 })
...
module.exports = {
  ...
  startBeldexdForRPC: startBeldexdForRPC,
  stopBeldexd: stopBeldexd,
