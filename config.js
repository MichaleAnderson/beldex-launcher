...
function getDefaultConfig(entrypoint) {
  ...
  const config = {
    launcher: {
      prefix: '/opt/beldex-launcher',
      //var_path: '/opt/beldex-launcher/var',
    },
    blockchain: {
      //binary_path: '/opt/beldex-launcher/bin/beldexd',
    },
  }
  ...
  // but what if we had both? we need to know which beldex-launcher is called
  if (entrypoint.match('/usr/bin')) {
    // apt install
    delete config.launcher.prefix // remove prefix
    config.blockchain.binary_path = '/usr/sbin/beldexd'
    config.launcher.var_path = '/var/lib/beldex-launcher'
  }
  return config
}
...
function getBeldexDataDir(config) {
  if (!blockchainDataDirReady) {
    console.trace('getBeldexDataDir is not ready for use!')
    process.exit(1)
  }
...
function loadBlockchainConfigFile(xmrOptions, config, output) {
  ...
  if (xmrOptions['config-file']) {
    // read file in beldexdata dir
    // FIXME: is it relative or absolute
    var filePath = xmrOptions['config-file']
    if (!fs.existsSync(filePath)) {
      var filePath2 = getBeldexDataDir(config) + '/' + xmrOptions['config-file']
      if (!fs.existsSync(filePath2)) {
        ...
        if (output) console.log('parsed beldex config', moneroDiskOptions.unknown, 'from', filePath2)
      ...
      const moneroDiskOptions = ini.iniToJSON(moneroDiskConfig.toString())
      if (output) console.log('parsed beldex config', moneroDiskOptions.unknown, 'from', filePath)
      for (var k in moneroDiskOptions.unknown) {
        var v = moneroDiskOptions.unknown[k]
        xmrOptions[k] = v
      }
    ...
    var defaultBeldexdConfigPath = getBeldexDataDir(config) + '/beldex.conf'
    if (fs.existsSync(defaultBeldexdConfigPath)) {
      const moneroDiskConfig = fs.readFileSync(defaultBeldexdConfigPath)
      const moneroDiskOptions = ini.iniToJSON(moneroDiskConfig.toString())
      if (output) console.log('parsed beldex config', moneroDiskOptions.unknown, 'from', defaultBeldexdConfigPath)
      for (var k in moneroDiskOptions.unknown) {
        var v = moneroDiskOptions.unknown[k]
        xmrOptions[k] = v
      }
...
function getOldBlockchainOptions(args, config, output) {
  ...
  // and ofc the beldexd.conf can override it all
...
function precheckConfig(config, args, debug) {
  ...
  if (config.launcher.prefix) {
    ...
    if (config.blockchain.binary_path === undefined) config.blockchain.binary_path = config.launcher.prefix + '/bin/beldexd'
    if (config.storage.binary_path === undefined) config.storage.binary_path = config.launcher.prefix + '/bin/beldex-storage'
    if (config.network.binary_path === undefined) config.network.binary_path = config.launcher.prefix + '/bin/belnet'
  }

  // we do need a var_path set for the all the PID stuff
  if (config.launcher.var_path === undefined) config.launcher.var_path = '/opt/beldex-launcher/var'
  ...
  // getBeldexDataDir should only be called after this point
...
function getBeldexdVersion(config) {
  if (config.blockchain.binary_path && fs.existsSync(config.blockchain.binary_path)) {
    try {
      var beldexd_version = lib.getBlockchainVersion(config)
      if (!beldexd_version.match) {
        // this could mean beldexd fails to run
        // (i.e. version `GLIBC_2.25' not found)
        // if we can't get it, maybe assume the latest?
        return false;
      }
      //console.log('beldexd_version', beldexd_version)
      binary3xCache = beldexd_version.match(/v3\.0/)?true:false
      binary4Xor5XCache = beldexd_version.match(/v[54]\./)?true:false
      binary7xCache = beldexd_version.match(/v7\./)?true:false
      return beldexd_version
    } catch(e) {
      console.error('Cant detect beldexd version', e)
...
function checkLauncherConfig(config) {
  ...
  if (config.blockchain.binary_path === undefined) config.blockchain.binary_path = '/opt/beldex-launcher/bin/beldexd'

  // only thing you can't turn off is blockchain (beldexd)
  // if you have storage without belnet, it will use the public IP to serve on
  // we will internally change these defaults over time
  // users are not encourage (currently) to put these in their INI (only beldex devs)
  if (config.network.enabled === undefined) {
    if (isBlockchainBinary3X(config) || isBlockchainBinary4Xor5X(config)) {
      console.log('3-5 series blockchain binary detected, disabling belnet by default')
      config.network.enabled = false
    } else {
      config.network.enabled = true
    }
    //console.log('belnet should be running?', config.network.enabled)
  }
  if (config.storage.enabled === undefined) {
    ...
    // I don't think we need this, 3.x users will be in the minority
     /* else
    if (is3x === undefined) {
      console.log('Could not detect your beldexd version, leaving storage server enabled')
    } */
  }
}
...
// i.e. storage/belnet restarts, we need to know the current ip if in auto-config mode
...
function setupInitialBlockchainOptions(xmrOptions, config) {
  ...
  if (xmrOptions['data-dir']) {
    ...
    if (!fs.existsSync(dir)) {
      console.warn('Configured data-dir [' + dir + '] does not exist, beldexd will create it')
    }
  }
...
function checkBlockchainConfig(config) {
  ...
  config.blockchain.beldexd_key = getBeldexDataDir(config) + '/key'
  config.blockchain.beldexd_edkey = getBeldexDataDir(config) + '/key_ed25519'
}
...
function checkNetworkConfig(config) {
  ...
  if (config.network.binary_path === undefined) config.network.binary_path = '/opt/beldex-launcher/bin/belnet'
  ...
  // putting all files required for migration into ~/.beldex/network
  if (config.network.data_dir === undefined) {
    // FIXME: really should be left alone and we should have a getter
    //console.log('default network server path, blockchain is', config.blockchain.data_dir)
    //config.network.data_dir = getBeldexDataDir(config) + '/network'
    config.network.data_dir = os.homedir() + '/.beldex/network'
  ...
  // if no bootstrap, set default (can't leave this blank for belnet)
  if (config.network.bootstrap_path === undefined && config.network.connects === undefined &&
     config.network.bootstrap_url === undefined) {
    if (config.network.testnet) {
      config.network.bootstrap_url = 'https://seed.belnet.org/testnet.signed'
    } else {
      config.network.bootstrap_url = 'https://seed.belnet.org/belnet.signed'
    }
  }
...
// we need beldexd_key configured for status mode
function checkStorageConfig(config) {
  if (!config.storage.enabled) return
  if (config.storage.binary_path === undefined) config.storage.binary_path = '/opt/beldex-launcher/bin/beldex-storage'
  ...
  if (config.storage.data_dir === undefined) {
    ...
    //config.storage.data_dir = getBeldexDataDir(config) + '/storage'
    // launcher can use the same storage path for testnet and mainnet
    config.storage.data_dir = os.homedir() + '/.beldex/storage'
  ...
  if (config.storage.beldexd_key === undefined) {
    config.storage.beldexd_key = getBeldexDataDir(config) + '/key'
  }
  config.storage.beldexd_rpc_port = config.blockchain.rpc_port
}
...
function changeHomedir(config, homedir) {
  if (config.blockchain.data_dir_is_default) {
    config.blockchain.data_dir = homedir + '/.beldex'
  }
  if (config.network.data_dir_is_default) {
    config.network.data_dir = homedir + '/.beldex/network'
    if (config.network.testnet) {
      config.network.data_dir += '_testnet'
    }
  }
  if (config.storage.data_dir_is_default) {
    config.storage.data_dir = homedir + '/.beldex/storage'
    if (config.storage.testnet) {
      config.storage.data_dir += '_testnet'
    }
  }
...
function ensureDirectoriesExist(config, uid) {
  ...
  if (!fs.existsSync(config.blockchain.data_dir)) {
    belnet.mkDirByPathSync(config.blockchain.data_dir)
    fs.chownSync(config.launcher.var_path, uid, 0)
  }
  // from belnet.js
  ...
  if (config.network.belnet_nodedb && !fs.existsSync(config.network.belnet_nodedb)) {
    console.log('making', config.network.belnet_nodedb)
    mkDirByPathSync(config.network.belnet_nodedb)
    fs.chownSync(config.launcher.var_path, uid, 0)
  }
}
...
function prequal(config) {
  // this port is set by default in beldexd
...
module.exports = {
  ...
  getBeldexDataDir: getBeldexDataDir,
  prequal: prequal,
